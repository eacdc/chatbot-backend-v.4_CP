...
router.post("/generate-final-prompt", authenticateAdmin, async (req, res) => {
  try {
    ...
    // Fetch the batchProcessing system prompt from the database for consistent formatting
    let finalPrompt;
    try {
      // First attempt to get the batchProcessing prompt for consistent format
      const batchPromptDoc = await Prompt.findOne({ prompt_type: "batchProcessing", isActive: true });
      let batchSystemPrompt = "";
      
      if (batchPromptDoc) {
        batchSystemPrompt = batchPromptDoc.prompt;
        console.log("Successfully loaded Batch Processing prompt for final prompt generation");
      } else {
        // Fallback to default batch prompt
        batchSystemPrompt = "You are a helpful assistant helping to understand and process educational content. Provide a comprehensive and detailed response to the text I will share with you. Focus on explaining the key concepts, identifying important information, and organizing the content in a structured way.";
        console.warn("Warning: Batch Processing prompt not found in database, using default");
      }
      
      // Now get the finalPrompt template
      const promptDoc = await Prompt.findOne({ prompt_type: "finalPrompt", isActive: true });
      if (promptDoc) {
        // Replace placeholders in the prompt
        const rawPromptTemplate = promptDoc.prompt
          .replace(/\${qnaOutput}/g, qnaOutput)
          .replace(/{subject}/g, subject || "")
          .replace(/{finalGrade}/g, finalGrade)
          .replace(/{chapterTitle}/g, chapterTitle);
          
        console.log("Successfully loaded Final Prompt template from database");
        
        // Add special instructions if provided
        const fullPromptTemplate = specialInstructions 
          ? `${rawPromptTemplate}\n\n${specialInstructions}`
          : rawPromptTemplate;
          
        // Process through batch processing format for consistency
        const response = await openai.chat.completions.create({
          model: "gpt-4.1",
          temperature: 0,
          messages: [
            { role: "system", content: batchSystemPrompt },
            { role: "user", content: fullPromptTemplate }
          ],
        });
        ...
        console.log("Final prompt processed through batch processing format for consistency");
      } else {
        // Fallback to default prompt if not found in DB
        console.warn("Warning: Final Prompt system prompt not found in database, using default");
        
        // Create a basic template combining QnA output with context
        const rawPromptTemplate = `Question Bank\n\n${qnaOutput}\n\nEnd of Question Bank\n\nYou are a teacher focusing on ${subject} for Grade ${finalGrade}. The chapter is ${chapterTitle}.`;
      ...
      console.error("Error processing final prompt:", error);
      
      // Fallback to direct template without processing
