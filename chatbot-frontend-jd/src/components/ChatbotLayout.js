...
export default function ChatbotLayout({ children }) {
  ...
  useEffect(() => {
    const fetchPublisherBooks = async () => {
      ...
      try {
        // Fetch books filtered by JD Editions publisher
        const booksResponse = await axios.get(`${API_ENDPOINTS.GET_BOOKS}?publisher=JD Editions`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        
        setPublisherBooks(booksResponse.data);
        console.log(`Fetched ${booksResponse.data.length} JD Editions books for carousel display`);
      } catch (error) {
