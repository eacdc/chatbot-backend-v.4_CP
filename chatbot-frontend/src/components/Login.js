...
const Login = () => {
  ...
  // Check if user is already logged in and prevent back navigation to login page
  useEffect(() => {
    const isAuthenticated = localStorage.getItem('isAuthenticated') === 'true';
    const token = localStorage.getItem('token');
    
    if (isAuthenticated && token) {
      // Replace current history entry with /chat to prevent back navigation to login
